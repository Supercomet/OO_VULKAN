...
#error Using XE_GTAO_USE_HALF_FLOAT_PRECISION with 32bit depths is not supported yet unfortunately (it is possible to apply fp16 on parts not related to depth but this has not been done yet)
		...
		#define lpfloat3x3 	min16float3x3   
		...
		#define  lpfloat 		float16_t      
		#define  lpfloat2		float16_t2     
		#define  lpfloat3		float16_t3     
		#define  lpfloat4		float16_t4     
		#define  lpfloat3x3		float16_t3x3   
	#endif
#else
...
{
    return lpfloat( asfloat( int(0x1fbd1df5 + ( asint( x ) >> 1 )) ));
}
...
void XeGTAO_DecodeVisibilityBentNormal( const uint packedValue, out lpfloat visibility, out lpfloat3 bentNormal )
{
    ...
    bentNormal = decoded.xyz * float3(2.0) - float3(1.0);   // could normalize - don't want to since it's done so many times, better to do it at the final step only
    visibility = decoded.w;
}

void XeGTAO_OutputWorkingTerm( const uint2 pixCoord, lpfloat visibility, lpfloat3 bentNormal, writeonly uimage2D outWorkingAOTerm )
{
    visibility = saturate( visibility / lpfloat(XE_GTAO_OCCLUSION_TERM_SCALE) );
#ifdef XE_GTAO_COMPUTE_BENT_NORMALS
	imageStore(outWorkingAOTerm, ivec2(pixCoord), ivec4(XeGTAO_EncodeVisibilityBentNormal( visibility, bentNormal ),0,0,0));
    //outWorkingAOTerm[pixCoord] = XeGTAO_EncodeVisibilityBentNormal( visibility, bentNormal );
#else
	imageStore(outWorkingAOTerm, ivec2(pixCoord), ivec4(visibility * 255.0 + 0.5) );
...
void XeGTAO_MainPass( const uint2 pixCoord, lpfloat sliceCount, lpfloat stepsPerSlice
    , const lpfloat2 localNoise, lpfloat3 viewspaceNormal
    , const GTAOConstants consts
	, texture2D sourceViewspaceDepth
	, sampler depthSampler	
	, writeonly uimage2D outWorkingAOTerm
	, writeonly image2D outWorkingEdges
	)
{                                                                       
    float2 normalizedScreenPos = (pixCoord + float2(0.5)) * consts.ViewportPixelSize;

	lpfloat4 valuesUL = textureGather(sampler2D(sourceViewspaceDepth,depthSampler),float2( pixCoord * consts.ViewportPixelSize ));
    //lpfloat4 valuesUL   = sourceViewspaceDepth.GatherRed( depthSampler, float2( pixCoord * consts.ViewportPixelSize )               );
    lpfloat4 valuesBR   = textureGatherOffset(sampler2D(sourceViewspaceDepth,depthSampler),float2( pixCoord * consts.ViewportPixelSize ), ivec2( 1, 1 ));
	...
	imageStore(outWorkingEdges,ivec2(pixCoord),vec4(XeGTAO_PackEdges(edgesLRTB)));
...
#if XE_GTAO_GENERATE_NORMALS_INPLACE
    ...
    // see "Algorithm 1" in https://www.activision.com/cdn/research/Practical_Real_Time_Strategies_for_Accurate_Indirect_Occlusion_NEW%20VERSION_COLOR.pdf
    {
        ...
        for( lpfloat slice = 0; slice < sliceCount; slice++ )
        {
            ...
            for( lpfloat step = 0; step < stepsPerSlice; step++ )
            {
                ...
                float  SZ0 = textureLod( sampler2D(sourceViewspaceDepth,depthSampler), sampleScreenPos0, mipLevel ).x;               
				...
				float  SZ1 = textureLod( sampler2D(sourceViewspaceDepth,depthSampler), sampleScreenPos1, mipLevel ).x;
...
, texture2D sourceNDCDepth
, sampler depthSampler
, writeonly image2D outDepth0
, writeonly image2D outDepth1
, writeonly image2D outDepth2
, writeonly image2D outDepth3
, writeonly image2D outDepth4 
)
{
    // MIP 0
    ...
    float4 depths4 = textureGatherOffset(sampler2D(sourceNDCDepth,depthSampler),float2( pixCoord * consts.ViewportPixelSize ),int2(1,1));
    ...
    imageStore(outDepth0, ivec2(pixCoord)+ int2(0, 0), vec4(lpfloat(depth0)) );
    imageStore(outDepth0, ivec2(pixCoord)+ int2(1, 0), vec4(lpfloat(depth1)) );
    imageStore(outDepth0, ivec2(pixCoord)+ int2(0, 1), vec4(lpfloat(depth2)) );
    imageStore(outDepth0, ivec2(pixCoord)+ int2(1, 1), vec4(lpfloat(depth3)) );

    // MIP 1
    lpfloat dm1 = XeGTAO_DepthMIPFilter( depth0, depth1, depth2, depth3, consts );
    //outDepth1[ baseCoord ] = (lpfloat)dm1;
    imageStore(outDepth1, ivec2(baseCoord), vec4(lpfloat(dm1)) );
    ...
    // MIP 2
    //[branch]
    if( all( equal(groupThreadID.xy % uvec2(2), uvec2(0)) ) )
    {
        ...
        imageStore(outDepth2, ivec2(baseCoord)/ 2, vec4(lpfloat(dm2)) );
    ...
    // MIP 3
    //[branch]   
    if( all( equal(groupThreadID.xy % uvec2(4), uvec2(0)) ) )
    {
        lpfloat inTL = g_scratchDepths[groupThreadID.x+0][groupThreadID.y+0];
        ...
        imageStore(outDepth3, ivec2(baseCoord)/ 4, vec4(lpfloat(dm3)) );
    ...
    // MIP 4
    //[branch]   
    if( all( equal(groupThreadID.xy % uvec2(8), uvec2(0)) ) )
    {
        lpfloat inTL = g_scratchDepths[groupThreadID.x+0][groupThreadID.y+0];
        ...
        imageStore(outDepth4, ivec2(baseCoord)/ 8, vec4(lpfloat(dm4)) );
...
#define AOTermType lpfloat4
#else
//typedef lpfloat AOTermType;             // .x is visibility term
#define AOTermType lpfloat
#endif
...
void XeGTAO_DecodeGatherPartial( uint4 packedValue, out AOTermType outDecoded[4] )
{
    for( int i = 0; i < 4; i++ )
#ifdef XE_GTAO_COMPUTE_BENT_NORMALS
...
, bool finalApply )
{
    ...
    lpfloat4 edgesQ0   = textureGatherOffset(sampler2D(sourceEdges,texSampler),gatherCenter, ivec2( 0, 0 ));
    lpfloat4 edgesQ1   = textureGatherOffset(sampler2D(sourceEdges,texSampler),gatherCenter, ivec2( 2, 0 ));
    lpfloat4 edgesQ2   = textureGatherOffset(sampler2D(sourceEdges,texSampler),gatherCenter, ivec2( 1, 2 ));
    ...
    for( int side = 0; side < 2; side++ )
    {
        ...
        lpfloat edginess = (saturate(4.0 - leak_threshold - dot( edgesC_LRTB[side], vec4(1) )) / (4-leak_threshold)) * leak_strength;
...
float3 XeGTAO_ComputeViewspaceNormal( uint2 pixCoord, GTAOConstants consts, texture2D sourceNDCDepth, sampler depthSampler )
{
    ...
    float4 valuesUL   = textureGather(sampler2D(sourceNDCDepth,depthSampler),float2( pixCoord * consts.ViewportPixelSize ));
    //float4 valuesBR   = sourceNDCDepth.GatherRed( depthSampler, float2( pixCoord * consts.ViewportPixelSize ), int2( 1, 1 ) );
    float4 valuesBR   = textureGatherOffset(sampler2D(sourceNDCDepth,depthSampler),float2( pixCoord * consts.ViewportPixelSize ), ivec2(1,1));
