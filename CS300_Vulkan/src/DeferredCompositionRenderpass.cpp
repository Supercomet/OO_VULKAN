...
void DeferredCompositionRenderpass::CreatePipeline()
{
	...
	rasterizationState.cullMode = VK_CULL_MODE_BACK_BIT;
	shaderStages[0] = VulkanRenderer::LoadShader(m_device, "Shaders/deferred.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
	shaderStages[1] = VulkanRenderer::LoadShader(m_device, "Shaders/deferred.frag.spv",VK_SHADER_STAGE_FRAGMENT_BIT);
	// Empty vertex input state, vertices are generated by the vertex shader
